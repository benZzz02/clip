...
# - temporal input: model-compatible window length
...
# module. Use NUM_FRAMES=32 only for encoders/checkpoints that support it.
...
export CACHE_DIR="${CACHE_DIR:-linear_probe_outputs/linear_probe_surglavi_protocol_nf${NUM_FRAMES}/cache}"
...
export RUN_NAME_PREFIX="${RUN_NAME_PREFIX:-linear_probe_surglavi_protocol_nf${NUM_FRAMES}/cls}"
export SHOT_MODE=cls
export SHOTS="${CLS_SHOTS:-1,2,4,8,16}"
bash ./run_linear_probe_sota_models.sh "$@"

export RUN_NAME_PREFIX="${RUN_NAME_PREFIX_VIDEO:-linear_probe_surglavi_protocol_nf${NUM_FRAMES}/video}"

...
export PROBE_FEATURE="${PROBE_FEATURE:-concat}"
export FRAME_STRIDE="${FRAME_STRIDE:-1}"
export SEEDS="${SEEDS:-0,1,2,3,4}"
# Standard frozen-feature linear probing follows the common CLIP-style setup:
# fit a regularized multinomial logistic-regression head to convergence.
export PROBE_SOLVER="${PROBE_SOLVER:-sklearn_logreg}"
export PROBE_STANDARDIZE="${PROBE_STANDARDIZE:-true}"
export SKLEARN_C="${SKLEARN_C:-1.0}"
export SKLEARN_MAX_ITER="${SKLEARN_MAX_ITER:-5000}"
export SKLEARN_SOLVER="${SKLEARN_SOLVER:-lbfgs}"
export SKLEARN_CLASS_WEIGHT="${SKLEARN_CLASS_WEIGHT:-none}"

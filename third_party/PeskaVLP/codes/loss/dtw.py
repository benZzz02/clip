    ...
    for i in range(0, nrows + 1):
        for j in range(0, ncols + 1):
            if (i == 0) and (j == 0):
                ...
                new_val = torch.finfo(sdtw.dtype).max
                sdtw = assign2Tensor(sdtw, i, j, new_val)
            elif (i != 0) and (j == 0):
                new_val = torch.finfo(sdtw.dtype).max
                sdtw = assign2Tensor(sdtw, i, j, new_val)
...
def compute_dtw_alignment_contrastive_loss(embs_v,
                                           embs_t,
                                           pos_indices,
                                           distance_type,
                                           softning,
                                           ...
                                           scale_factor,
                                           cyclic=False):
    # embs_v - [batch_size, num_steps_v, dim]
    # embs_t - [batch_size, num_steps_t, dim]
    batch_size = embs_v.shape[0]
    logits_list = []
    loss_list = []
    for i in range(batch_size):
        embs1 = embs_v[i]
        ...
        mask = pos_indices[i]
        mask = mask[mask!=-1]
        embs2 = embs2[mask] # text sequence might have padding
        ...
        loss_diff = torch.maximum(a - b + beta, torch.zeros_like(a))
...
def compute_dtw_loss(embs_v,
    ...
    #                                       gamma_s=gamma_s,
    #                                       gamma_f=gamma_f)
    #     return dtw_scale_factor * loss, torch.zeros_like(loss)
    ...
    if alignment_type == 'dtw_contrastive':
        loss, loss2 = compute_dtw_alignment_contrastive_loss(embs_v=embs_v,
                                                             embs_t=embs_t,
                                                             ...
                                                             gamma_f=gamma_f,
                                                             beta=dtw_beta,
                                                             loss_ratio=dtw_ratio,
                                                             scale_factor=dtw_scale_factor,

...
class SSL_VL_Loss_new(nn.Module):
    def __init__(self, temperature=0.1, alpha_weight=0.75):
        super(SSL_VL_Loss_new, self).__init__()
    ...
    def forward(self, video_embd, video_embd_aug1, video_embd_aug2, text_embds, logit_scale=None):
        ...
        loss_1_vl = self.clip_info_loss(video_embd, text_embds[0])
        loss_2_vl = self.ntx_loss(video_embd, text_embds[0])
        loss_3_vl = self.mil_nce_loss(video_embd, text_embds[1])
        loss_1 = (loss_1_vl + loss_2_vl + loss_3_vl) / 3

        # Augmented Image_1 - Text Supervision
        loss_1_vl = self.clip_info_loss(video_embd_aug1, text_embds[0])
        loss_2_vl = self.ntx_loss(video_embd_aug1, text_embds[0])
        loss_3_vl = self.mil_nce_loss(video_embd_aug1, text_embds[1])
        loss_2 = (loss_1_vl + loss_2_vl + loss_3_vl) / 3

        # Augmented Image_2 - Text Supervision
        loss_1_vl = self.clip_info_loss(video_embd_aug2, text_embds[0])
        loss_2_vl = self.ntx_loss(video_embd_aug2, text_embds[0])
        loss_3_vl = self.mil_nce_loss(video_embd_aug2, text_embds[1])

...
@DATASETS.register_module(name='video_text_ssl')
class Video_Text_Loader(Dataset):
    ...
    def _get_video(self, video_path, start, end):
        ...
        # Perform Augmentation for SSL
        aug1_list = []
        aug2_list = []
        for img in video:
            aug1 = self.augment(img)
            aug2 = self.augment(img)
            aug1_list.append(aug1.unsqueeze(0))
            aug2_list.append(aug2.unsqueeze(0))
        aug1_video = torch.cat(aug1_list, 0)
        aug2_video = torch.cat(aug2_list, 0)
        # utils.save_image(aug1_video, 'tmp.png')

        video = video / 255.0
        video = self.transforms(video)

        return video, aug1_video, aug2_video
    ...
    def __getitem__(self, idx):
        ...
        for idx, asr_root in enumerate(self.caption_roots):
            ...
            if idx == 0:
                ...
                # get video
                video, aug1_video, aug2_video = self._get_video(video_path, start, end)
            else:
        ...
        final_dict['video_aug1'] = aug1_video
        final_dict['video_aug2'] = aug2_video

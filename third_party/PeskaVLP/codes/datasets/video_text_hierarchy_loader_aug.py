...
@DATASETS.register_module(name='video_text_phase_aug')
class Video_Text_Loader(Dataset):
    ...
    def _get_text(self, caption, num_texts):
        ...
        if 'step' in cap.columns:  # Check if 'step' column exists
            selected_column = random.choice(['step', 'text'])  # Randomly select 'step' or 'text'
        else:
    ...
    def _find_nearest_candidates(self, start, end, caption):
        ...
        if 'step' in caption.columns:  # Check if 'step' column exists
            selected_column = random.choice(['step', 'text'])  # Randomly select 'step' or 'text'
        else:
            selected_column = 'text'  # Default to 'text' column if 'step' doesn't exist
        ...
        captions = pd.concat([df1, df2, df3], axis=0)[selected_column].values[:].tolist()
    ...
    def __getitem__(self, idx):
        ...
        if pad > 0:
            ...
            # indices = list(range(0, len(samples), step))[:self.candidates[0]]
